<div class="p-4">
    <h1 class="text-base font-semibold md:text-2xl">Share Reimbursement Application (APPROVER)</h1>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4">
                ...
                <x-form.dropdown
                    ...
                    @foreach ($banks ?? [] as $bank)
                        <option @if ($bank->code == $approver->bank_code) selected @endif>{{ $bank->description }}</option>
                    @endforeach
                </x-form.dropdown>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
                        <x-form.input
                            label="Member Name"
                            name="buyer_name"
                            value="{{ $approver->exc_cust_id == NULL ? '' : $approver->buyer->name ?? '' }}"
                            mandatory=""
                            disable="true"
                            type="text"
                        />
                        ...
                        <x-form.input
                            label="Member IC No."
                            name="buyer_icno"
                            value="{{ $approver->exc_cust_id == NULL ? '' : $approver->buyer->icno ?? '' }}"
                            mandatory=""
                            disable="true"
                            type="text"
                        />
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4">
                ...
                <div>
                    <x-form.input
                        ...
                        value="{{ $approver->direction == 'sell' ? 'Co-operative' : 'Member' }}"
                        name="share_type"
                        id="share_type"
                        mandatory=""
                        disable="true"
