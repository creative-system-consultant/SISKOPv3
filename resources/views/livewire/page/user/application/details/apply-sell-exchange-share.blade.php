        ...
        <div class="pb-4 pl-4 pr-4">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4">
                <x-form.input
                    label="Name"
                    name="custname"
                    value="{{ $sell_share->customer->name ?? '' }}"
                    mandatory=""
                    disable="true"
                    type="text"
                />
                ...
                <x-form.input
                    ...
                    name="custic"
                    value="{{ $sell_share->customer->icno ?? '' }}"
                    mandatory=""
                    disable="true"
                    type="text"
                />
                ...
                <x-form.dropdown
                    ...
                    @foreach ($banks ?? [] as $bank)
                        <option @if ($bank->code == $sell_share?->bank_code) selected @endif>{{ $bank->description }}</option>
                    @endforeach
                </x-form.dropdown>
            ...
            @if(isset($sell_share->direction) && $sell_share->direction !== 'sell'  )
            <div>
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
                        <x-form.input
                            label="Member Name"
                            name="buyer_name"
                            value="{{ isset($sell_share->exc_cust_id) && $sell_share->exc_cust_id == NULL ? '' : $sell_share->buyer->name ?? '' }}"
                            mandatory=""
                            disable="true"
                            type="text"
                        />
                        ...
                        <x-form.input
                            label="Member IC No."
                            name="buyer_icno"
                            value="{{ isset($sell_share->exc_cust_id) && $sell_share->exc_cust_id == NULL ? '' : $sell_share->buyer->icno ?? '' }}"
                            mandatory=""
                            disable="true"
                            type="text"
                        />
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4">
                ...
                <div>
                    <x-form.input
                        ...
                        value="{{ isset($sell_share->direction) && $sell_share->direction == 'sell' ? 'Co-operative' : 'Member' }}"
                        name="share_type"
                        id="share_type"
                        mandatory=""
                        disable="true"
