<div class="p-4">
    ...
    <x-general.card class="relative py-4 mt-4 rounded-md shadow-md "  x-data="{ active:0 }"
    @tab-changed.window="active = $event.detail.newActiveTab"
    @increment-tab.window="active++"
    @decrement-tab.window="if(active > 0) active--">
        <div class="flex flex-wrap justify-start px-4 mx-4 bg-white rounded-lg shadow-lg sm:justify-start dark:bg-gray-800">
            {{-- <p>Active Tab: <span x-text="active"></span></p> --}}
            <x-hovertab.title name="0" varble="{{$tab1}}" wire:click="nextTab({{$numpage}},1,1)">
                <x-heroicon-o-user-circle class="w-6 h-6 " />
                <span class="text-sm text-white border rounded tooltip-text bg-primary-400 border-primary-500 -mt-14">
                    Personal Detail
                </span>
            </x-hovertab.title>
